...
def save_to_database(model_id, model_desc, db, model, X_train, X_test, y_train, y_test):
    ...
    for i, val in enumerate(model_coef):
        ...
        cursor.execute('''INSERT INTO model_coefs 
                  (id, desc, feature_name, value) VALUES (?, ?, ?, ?)''', model_coef_to_insert)
    ...
    cursor.execute('''INSERT INTO model_results 
          (id, desc, train_score, test_score) VALUES (?, ?, ?, ?)''', model_results_to_insert)
...
feature_cols = ['mean radius', 'texture error', 'worst radius', 'worst compactness', 'worst concavity']
